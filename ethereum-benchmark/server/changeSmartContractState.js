...
const setInterbankTransfer = () => {
  console.log(
    "#######################Preparing InterbankTransfer.transfer() stress test#######################"
  );
  const InterbankTransferABI = require('./smartContract/build/InterbankTransfer/InterbankTransfer.json').abi
  const contractAddress = process.env.SMART_CONTRACT_ADDRESS || process.env.INTERBANK_CONTRACT_ADDRESS
...
const chooseSmartContractSetter = () => {
  ...
  } else if (SMART_CONTRACT_OPTION == 5) {
    set = setInterbankTransfer;
  }
};
